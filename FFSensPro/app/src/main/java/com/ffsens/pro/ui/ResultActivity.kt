...
class ResultActivity : AppCompatActivity() {
    ...
    private fun loadResults() {
        ...
        val deviceName = intent.getStringExtra("DEVICE_NAME") ?: "Your Device"
        val ffVersion  = intent.getStringExtra("FF_VERSION") ?: "Not detected"
        val ffMod      = intent.getBooleanExtra("FF_SUSPECTED_MOD", false)
        val tips       = intent.getStringArrayListExtra("TIPS") ?: arrayListOf()

        binding.tvDeviceName.text = deviceName
        binding.tvCalScore.text   = "$calScore% Calibrated"
        ...
        when {
            ffMod -> {
                binding.tvFfVersion.text = "⚠️ Unofficial FF: $ffVersion"
                binding.tvFfVersion.setBackgroundResource(R.drawable.badge_warning)
            }
            ffVersion == "Not installed" -> {
                binding.tvFfVersion.text = "ℹ️ FF Not Installed"
                binding.tvFfVersion.setBackgroundResource(R.drawable.badge_info)
            }
            else -> {
                binding.tvFfVersion.text = "✅ Free Fire $ffVersion"
        ...
        binding.barGeneral.progress    = prog(general)
        binding.barRedDot.progress     = prog(redDot)
        binding.barScope2x.progress    = prog(scope2x)
        binding.barScope4x.progress    = prog(scope4x)
        binding.barSniper.progress     = prog(sniper)
        binding.barFreeRecoil.progress = prog(freeRecoil)
        ...
        binding.btnCopy.tag = buildShareText(
            general, redDot, scope2x, scope4x, sniper, freeRecoil, lookJoy, fireBut, deviceName)
    }
    ...
    private fun setupButtons() {
        ...
        binding.btnCopy.setOnClickListener {
            ...
            cm.setPrimaryClip(ClipData.newPlainText("FF Sensitivity", text))
            Toast.makeText(this, "✅ Sensitivity copied!", Toast.LENGTH_SHORT).show()
        }
    }

    private fun buildShareText(
        general: Int, redDot: Int, scope2x: Int, scope4x: Int,
        sniper: Int, freeRecoil: Int, lookJoy: Int, fireBut: Int, device: String
    ) = """
🔥 FF SENS PRO — Calibrated Settings
📱 Device: $device
━━━━━━━━━━━━━━━━━━━━
🎯 General:        $general
🔴 Red Dot:        $redDot
...
🔭 4x Scope:       $scope4x
🎯 Sniper (8x):    $sniper
💥 Free Recoil:    $freeRecoil
🕹️  Look Joystick: $lookJoy
🔫 Fire Button:    $fireBut
━━━━━━━━━━━━━━━━━━━━
Generated by FF Sens Pro
    """.trimIndent()
