...
class MainActivity : AppCompatActivity() {
    ...
    private fun updateDevicePreview(specs: DeviceSpecs) {
        ...
        binding.tvDeviceDetails.text = buildString {
            ...
            append(" · ${specs.cpuCores} cores\n")
            append("RAM: ${specs.totalRamMB / 1024}GB total · ${specs.availableRamMB}MB free\n")
            append("Screen: ${specs.screenWidthPx}×${specs.screenHeightPx} · ${specs.densityDpi}dpi\n")
            append("Display: ${"%.1f".format(specs.screenInches)}\" · ${specs.refreshRateHz.toInt()}Hz")
        }

        // Free Fire status chip
        if (specs.ffInstalled) {
            binding.chipFfStatus.text = if (specs.ffSuspectedMod)
                "⚠️ FF Installed (Unofficial)"
            else
                "✅ Free Fire v${specs.ffVersionName}"
            binding.chipFfStatus.setChipBackgroundColorResource(
                if (specs.ffSuspectedMod) R.color.ff_warning else R.color.ff_success
            )
        } else {
            binding.chipFfStatus.text = "❌ Free Fire Not Found"
    ...
    private fun startScan() {
        ...
        val scanSteps = listOf(
            "Reading CPU specs...",
            "Measuring screen metrics...",
            "Analyzing RAM profile...",
            "Detecting Free Fire...",
            "Calibrating sensitivity engine...",
